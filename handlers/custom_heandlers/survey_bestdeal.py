...
def get_town(message: Message) -> None:
    if message.text.isalpha():
        if get_areas(message.text):
            ...
            bot.set_state(
                message.from_user.id, BestdealInfoState.town_area, message.chat.id
            )
            with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
...
def get_check_in(message: Message) -> None:
    if check_data_in(message.text):
        ...
        bot.set_state(
            message.from_user.id, BestdealInfoState.check_out, message.chat.id
        )
...
def get_check_out(message: Message) -> None:
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        if check_data_out(message.text, data["check_in"]):
            bot.send_message(
                message.from_user.id, "Введите минимальную цену номера в отеле"
            )
            bot.set_state(
                message.from_user.id, BestdealInfoState.min_price, message.chat.id
            )
...
def get_min_price(message: Message) -> None:
    if message.text.isdigit():
        bot.set_state(
            message.from_user.id, BestdealInfoState.max_price, message.chat.id
        )
        bot.send_message(
            message.from_user.id, "Введите максимальную цену номера в отеле"
        )
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
    ...
    if message.text.isdigit():
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            if int(message.text) >= data["min_price"]:
                ...
                bot.set_state(
                    message.from_user.id, BestdealInfoState.min_dist, message.chat.id
                )
                bot.send_message(
                    message.from_user.id,
                    "Введите минимальное расстояние до центра города в км",
                )
            else:
                bot.send_message(
                    message.from_user.id,
        ...
        bot.send_message(
            message.from_user.id, "Максимальная цена это натуральное число"
        )
    ...
    try:
        ...
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            if float(message.text) >= data["min_dist"]:
                bot.set_state(
                    message.from_user.id, BestdealInfoState.num_hotels, message.chat.id
                )
                bot.send_message(
                ...
                bot.send_message(
                    message.from_user.id,
                    "Максимальное расстояние не может быть меньше минимального",
                )
...
def get_num_hotels(message: Message) -> None:
    if message.text.isdigit() and int(message.text) > 0:
        ...
        bot.set_state(
            message.from_user.id, BestdealInfoState.photo_choise, message.chat.id
        )
...
def callback_if_need_photo(message: Message) -> None:
    with bot.retrieve_data(message.from_user.id) as data:
        ...
        text = (
            "Вам нужно вывести {num_hotels} отеля c ценами {min_price} - {max_price} руб"
            " и расстоянием до центра города {min_dist} - {max_dist} км?".format(
...
def get_num_photo(message: Message) -> None:
    if message.text.isdigit():
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            ...
            text1 = (
                "Вам нужно вывести {num_hotels} отеля с {num_photo} фото c ценами {min_price} - {max_price} руб"
                " и расстоянием до центра города {min_dist} - {max_dist} км?".format(
                    num_hotels=data["num_hotels"],
        ...
        bot.set_state(
            message.from_user.id, BestdealInfoState.put_results, message.chat.id
        )
